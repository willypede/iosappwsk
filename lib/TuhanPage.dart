...
class TuhanPage extends StatefulWidget {
  List? list;
  late int index;

  @override
  _TuhanPageState createState() => _TuhanPageState();
}

class _TuhanPageState extends State<TuhanPage> {
  void initState() {
    super.initState();
  ...
  print("before");
  final response = await http.post(Uri.parse("https://www.warungsatekamu.org/wsk_app_2020/load1Tuhan.php",
  ), body: {
  });
  ...
  Future<List?> getTuhan() async {
  print("before");
  final response = await http.post(Uri.parse("https://warungsatekamu.org/wsk_app_2020/loadTuhan.php",
  ), body: {
    "limit": "0"
  });
  print("hasil "+response.body.toString());
